...
INSTALL_LOG="/tmp/thyme_streamlined_install.log"
...
EOF
    ...
    echo "Thyme OS Streamlined Installer Log - $(date)" > "$INSTALL_LOG"
...
smart_copy_mint_system() {
    ...
    # Define packages to exclude DURING copy (not after)
    local bloatware_patterns=(
        ...
        # Heavy language packs (keep English)
        "*language-pack-[^e]*"
        "*hunspell-[^e]*"
        "*aspell-[^e]*"
        "*mythes-[^e]*"
